...
tab_control.add(evaldef, text='Evaluation Results')
...
evalboxes=[] #list of checkboxes for evaluation
...
def modelbtnclick():
    ...
    for k,v in ODEtext.items():
        check=IntVar()
        evalboxes.append(Checkbutton(moddef,variable=check))
        evalboxes[i-1].var=check
        evalboxes[i-1].grid(column=1,row=i)
...
def residual (ps,ts,data):
    model=g(ts,x0,ps)
    #rewrite residuals without missing eval data
    i=len(evalboxes)
    for item in reversed(evalboxes):
        i=i-1
        if item.var.get() == 0:
            model=np.delete(model,i,axis=1)
    return(model-data).ravel()

#Evaluation Results Definition

def pickeval():
    #getfile
    compartments=[]
    evalfilename=filedialog.askopenfilename(filetypes = (("CSV","*.csv"),("all files","*.*")))
    with open(evalfilename, newline='') as evalfile:
        evalreader = csv.reader(evalfile)
        global t
        global expdata
        t=np.loadtxt(next(evalreader),dtype='int16',delimiter=',')
    ...
    #show eval results
    c=1
    Label(evaldef, text='Compart', width=10, justify='left').grid(column=0,row=2)
    Label(evaldef, text='Time', width=10, justify='left').grid(column=1,columnspan=t.size,row=1)
    for time in t:
        Label(evaldef, text=time, width=10, justify='left').grid(column=c,row=2)
        c=c+1
    r=3
    for comp in compartments:
        Label(evaldef, text=comp, width=10, justify='left').grid(column=0,row=r)
        r=r+1
    r=3
    for comp in expdata:
        c=1
        for mass in comp:
            Label(evaldef, text=mass, width=10, justify='left').grid(column=c,row=r)
            c=c+1
        r=r+1

pickevalbtn = Button(evaldef, text="Pick Eval File", command=pickeval)
pickevalbtn.grid(column=0,row=0)
